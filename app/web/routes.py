...
from app.models.user_models import UserCreate, UserLogin, ProductCreate, CategoryCreate
...
def get_admin_user(token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(start_db)):
    credentials_exception = HTTPException(
        status_code=401,
        detail="Could not validate credentials",
        headers={"WWW-Authenticate": "Bearer"})
...
async def register(user: UserCreate, db: Session = Depends(start_db)):
    ...
    if user.username == ADMIN_USERNAME and user.password == ADMIN_PASSWORD:
        n_user.is_admin = True
    else:
        n_user.is_admin = False
    db.add(n_user)
    db.commit()
...
@app.get('/')
async def products(page: int, db: Session = Depends(start_db), length: int = 20):
    offset = (page-1) * length
    to_show = db.query(Product).offset(offset).limit(length).all()
    return to_show

@app.get('/category')
async def products(page: int, db: Session = Depends(start_db), length: int = 20):
    offset = (page-1) * length
    to_show = db.query(Category).offset(offset).limit(length).all()
    return to_show
...
@app.delete("/del-product/{product_id}")
async def delete_product(
    ...
    try:
        ...
        if not product:
            ...
            db.delete(product)
            db.commit()
            return {"msg": f'Deleted product with id {product.id} successfully'}
...
@admin_app.post('/add-category')
def new_category(category: CategoryCreate, db: Session = Depends(start_db), user = Depends(get_admin_user)):
    possible_db_category = db.query(Category).filter(category.name == Category.name).first()
    if possible_db_category:
        raise HTTPException(status_code=400, detail="Category already exist")
    new_ctg = Category(name = category.name)
    db.add(new_ctg)
    db.commit()
    return {'Msg': 'added'}
